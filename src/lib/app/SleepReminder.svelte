...
{#if visible}
	...
	>
		...
		<div class="min-w-0 flex-1 pt-0.5">
			<p class="font-display text-base font-semibold text-moss-dark">A note from the grove</p>
			<p class="mt-1 text-sm leading-relaxed text-ink-muted">
				The grove will be here tomorrow. Remember to leave a little time for rest, too.
			</p>
		</div>
		<button
			class="grid size-10 shrink-0 place-items-center rounded-xl text-ink-muted transition hover:bg-mist hover:text-moss"
			type="button"
			aria-label="Dismiss sleep reminder"
