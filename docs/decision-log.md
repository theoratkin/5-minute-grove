...
- Added optional desktop notifications for completed 5-minute blocks, requested from Start/Add 5 user actions and treated as nonessential when unsupported or denied.
...
- Moved the persisted theme selector into a focused settings modal and added a concise About page centered on permission to stop.
...
- Added durable active-session recovery using an absolute segment end time, including honest completion when the timer expires while the page is suspended or closed.
- Split natural timer completion from ending a session early. Partial work is now saved without incrementing the completed-block count.
- Expanded the end-of-timer decision into Add 5 minutes, Finish here, Take a break, and Switch task while preserving Add 5 as the primary invitation.
...
- Added a development-only `F` shortcut that fast-forwards a running or paused timer through the normal completion path for testing the finish UI.
...
- Expanded the windowsill vignette into a persistent local grove. Every completed five-minute block adds one leaf, 50 leaves mature a tree, and mature trees accumulate without streaks, decay, currency, or reversal when history is deleted. Growth is credited idempotently per session and backfilled once from retained local records.

## 2026-07-17

- Increased grove growth to five leaves per completed five-minute block—one leaf per completed minute, awarded together when the timer finishes. Existing credited blocks migrate to the new rate without double-counting.
...
- Renamed the product from "Just 5 More Minutes" to "Five Minute Grove" to reflect its calm, cumulative growth metaphor while preserving the five-minute starting point.
...
- Opened the grove vignette into a fully outdoor, edge-to-edge landscape. The interior window frame, sill, and furniture were removed; the mug appears separately as a quiet ritual cue only when a five-minute block is complete.
...
- Added a first-visit introduction that briefly distinguishes the app's approachable starting point and voluntary +5 loop from a conventional timer. Dismissal is stored locally, and a development-only `Shift+I` shortcut reopens it from any route for testing.
...
- Replaced Anything's assignment-only swap control with a standard actions menu. Assignment remains the primary section, while an undoable “Discard {time}” action removes the unassigned task aggregate without taking away earned grove growth; discard waits until any active Anything attempt has stopped.
- Chose GitHub Pages for static production hosting at `5minutegrove.com`. Added a checked GitHub Actions deployment, the SvelteKit static adapter, trailing-slash prerendered routes, and a client fallback for direct localized URLs.
