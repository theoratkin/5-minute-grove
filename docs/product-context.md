...
The timer includes a local, persistent grove as a living record of focused time. Each completed minute adds one leaf to the current tree in real time. A tree matures at 60 leaves—one focused hour—and remains in the foreground until the user starts a new session; it then joins the background grove and makes room for another tree.
