...
"5 Minute Grove" is a focus web app centered on approachable starts, voluntary continuation, and a grove that grows with focused time.

The user found that ordinary Pomodoro, stopwatch, and timer workflows do not work well for them because getting started is the hardest part. A 5-minute timer works better because it offers a small, believable starting point with permission to quit afterward. In Virtual Cottage 2, the end-of-timer "Add 5 minutes" button creates a useful loop: once started, clicking another 5 minutes is easy enough to keep momentum going.

## Product Thesis

The product is not primarily a timer. It is a momentum loop:

Choose an approachable starting time -> focus until it ends -> allow stopping -> invite +5 -> repeat.
...
Users may choose a shorter or longer opening time when that makes starting feel more approachable or lets the app serve as a conventional timer. The voluntary five-minute continuation remains the central invitation when that chosen time ends.
...
The grove is a keepsake rather than an economy: it has no streaks, decay, spending, optimization, or missed-day states. Completed whole minutes remain honestly represented even when a session ends before its chosen time; incomplete minutes do not add leaves, and deleting a session-history entry does not erase growth that already happened.
...
- Do not describe a partial block as a finished five-minute block; encouragement should remain truthful.
